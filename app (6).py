...
def build_ffmpeg_args(filename, fmt, out_filename, tolerant=False):
    src = f"/input/upload-file/{filename}"
    # tolerant=True শুধু রিট্রাই-এ ব্যবহার হয়: বড় দৈর্ঘ্যের ফোনে-রেকর্ড করা ভিডিওতে মাঝপথে
    # কোনো ফ্রেম/টাইমস্ট্যাম্প corrupt থাকলে ffmpeg ডিফল্টভাবে সেখানেই থেমে যায় এবং একটা
    # ছোট কিন্তু "valid" আউটপুট ফাইল তৈরি করে ফেলে (এটাই ৩০ মিনিটের ভিডিও থেকে মাত্র
    # কয়েক সেকেন্ডের আউটপুট আসার আসল কারণ) — এই ফ্ল্যাগগুলো সেই corrupt অংশ স্কিপ করে
    # বাকি পুরো ভিডিও প্রসেস চালিয়ে যেতে বাধ্য করে।
    extra = "-fflags +genpts+igndts+discardcorrupt -err_detect ignore_err " if tolerant else ""
    if fmt == "3gp":
        return (
            f"-i {src} {extra}-map 0:v:0 -map 0:a:0? -vf scale=176:144 -vcodec h263 -b:v 128k -r 15 "
            f"-acodec aac -ar 8000 -ac 1 -b:a 32k -max_muxing_queue_size 9999 "
            f"-avoid_negative_ts make_zero -y /output/{out_filename}"
        )
    if fmt == "mp3":
        return (
            f"-i {src} {extra}-map 0:a:0 -vn -acodec libmp3lame -ar 44100 -b:a 128k "
...
def parse_ffmpeg_durations(log_text):
    """CloudConvert-এর capture_output লগ থেকে সোর্স ভিডিওর আসল Duration এবং ffmpeg
    সবশেষ পর্যন্ত কতটুকু প্রসেস করেছে (শেষ 'time=' এন্ট্রি) — দুটোই সেকেন্ডে বের করে।
    কোনোটা লগে না পাওয়া গেলে None রিটার্ন করে (তখন caller size-ভিত্তিক ফলব্যাকে যায়)।"""
    if not log_text:
        return None, None
    ...
    dur_match = re.search(r"Duration:\s*(\d+):(\d+):(\d+\.\d+)", log_text)
    input_seconds = to_seconds(*dur_match.groups()) if dur_match else None

    time_matches = re.findall(r"time=(\d+):(\d+):(\d+\.\d+)", log_text)
    output_seconds = to_seconds(*time_matches[-1]) if time_matches else None

    return input_seconds, output_seconds


def cloudconvert_convert(input_path, filename, fmt, progress_cb=None, tolerant=False):
    ...
    ffmpeg_args = build_ffmpeg_args(filename, fmt, out_filename, tolerant=tolerant)
    ...
    job_payload = {
        "tasks": {
            ...
            "convert-file": {
                ...
                "capture_output": True,  # ffmpeg-এর আসল লগ পাওয়া যায়, যা দিয়ে output duration যাচাই করা হয়
                "timeout": 7200,
            },
    ...
    convert_task = next(t for t in job["tasks"] if t["name"] == "convert-file")
    log_text = (convert_task.get("result") or {}).get("output") or ""
    input_seconds, output_seconds = parse_ffmpeg_durations(log_text)
    ...
    dl.raise_for_status()
    return out_name, dl.content, input_seconds, output_seconds
...
def background_convert(job_id, uid, input_path, filename, fmt, source_label):
    try:
        ...
        out_name = content = None
        input_seconds = output_seconds = None
        is_short = False

        # সর্বোচ্চ ২ বার চেষ্টা করা হয়: প্রথমবার স্বাভাবিক ffmpeg কমান্ড, আউটপুট যদি সোর্সের
        # তুলনায় অস্বাভাবিক রকম ছোট (< ৮৫%) হয় — তাহলে দ্বিতীয়বার corrupt/timestamp-tolerant
        # ফ্ল্যাগ দিয়ে আবার চেষ্টা করা হয় (দীর্ঘ, ফোনে-রেকর্ড করা ভিডিওতে এটাই মূল কারণ)
        for attempt, tolerant in enumerate((False, True)):
            if attempt > 0:
                JOBS[job_id]["stage"] = "retrying"
                JOBS[job_id]["progress"] = 0
            out_name, content, input_seconds, output_seconds = cloudconvert_convert(
                input_path, filename, fmt, progress_cb, tolerant=tolerant
            )

            duration_known = input_seconds is not None and output_seconds is not None
            if duration_known:
                is_short = input_seconds > 8 and output_seconds < input_seconds * 0.85
            else:
                # ffmpeg লগ থেকে duration না পাওয়া গেলে পুরনো size-ভিত্তিক ফলব্যাক ব্যবহার হয়
                is_short = input_size > 5 * 1024 * 1024 and len(content) < max(60 * 1024, input_size * 0.05)

            if not is_short:
                break

        if is_short:
            if input_seconds is not None and output_seconds is not None:
                detail = f"আসল ভিডিও {human_time(input_seconds)} কিন্তু কনভার্ট হয়েছে মাত্র {human_time(output_seconds)}"
            else:
                detail = "আউটপুট ফাইল সোর্সের তুলনায় অস্বাভাবিক রকম ছোট"
            raise RuntimeError(
                f"ভিডিওটি বারবার মাঝপথে থেমে যাচ্ছে ({detail}) — সম্ভবত ফাইলের কোনো অংশ Corrupt। "
                f"অন্য একটি ভিডিও দিয়ে চেষ্টা করো, বা ফাইলটি একবার Trim/Re-export করে আবার Upload করে দেখো।"
            )
        ...
        # আগে হিস্ট্রিতে যোগ করে DOWNLOAD_INDEX-এ রেজিস্টার করা হচ্ছে, তারপর Telegram-এ পাঠানো
        # হচ্ছে — এই ক্রম উল্টো থাকলে বটে পাঠানো লিংকে কেউ সাথে সাথেই ক্লিক করলে
        # "ফাইল খুঁজে পাওয়া যায়নি" দেখানোর ছোট্ট একটা race condition তৈরি হতো
        add_history(uid, entry)

        sent = False
        chat_id = telegram_chat_id_for(uid)
        if chat_id:
            # WEBSITE_URL + /dl/<id> — এটা কোনো লগইন/সেশন ছাড়াই সরাসরি খোলা যায়,
            # তাই ৫০MB-র বেশি ফাইলে (বা পাঠাতে সমস্যা হলে) এই লিংকটাই বটে ক্লিকযোগ্য আকারে পাঠানো হয়
            full_download_url = f"{WEBSITE_URL.rstrip('/')}/dl/{entry_id}"
            sent = send_document_to_telegram(chat_id, out_path, download_url=full_download_url)
...
def send_document_to_telegram(chat_id, filepath, download_url=None):
    try:
        if os.path.getsize(filepath) > TELEGRAM_MAX_UPLOAD:
            link_line = f"\n\n⬇️ ডাউনলোড লিংক:\n{download_url}" if download_url else " অ্যাপ থেকে সরাসরি ডাউনলোড করো।"
            send_message(chat_id, f"⚠️ কনভার্ট হওয়া ফাইলটি ৫০MB-র বেশি, তাই বটে সরাসরি পাঠানো যায়নি।{link_line}")
            return False
        ...
        if not ok:
            link_line = f"\n\n⬇️ ডাউনলোড লিংক:\n{download_url}" if download_url else " অ্যাপ থেকে ডাউনলোড করে নাও।"
            send_message(chat_id, f"⚠️ কনভার্ট হওয়া ফাইলটি বটে পাঠাতে সমস্যা হয়েছে।{link_line}")
        return ok
    except Exception:
        return False
...
<style>
  ...
  *{box-sizing:border-box; -webkit-tap-highlight-color:transparent}
  ...
  .source-btn .ic{width:42px;height:42px;border-radius:13px;margin:0 auto 7px;font-size:19px;
    display:flex;align-items:center;justify-content:center;
    background:linear-gradient(155deg,rgba(255,255,255,.1),rgba(255,255,255,.03));
    border:1px solid var(--border-hi); box-shadow:inset 0 1px 0 rgba(255,255,255,.18); transition:all .2s ease}
  .source-btn.active .ic{background:rgba(255,255,255,.22); border-color:rgba(255,255,255,.4);
    box-shadow:inset 0 1px 0 rgba(255,255,255,.35), 0 4px 10px rgba(0,0,0,.15)}
  ...
  .fmt-btn .ic{width:46px;height:46px;border-radius:15px;margin:0 auto 8px;font-size:22px;
    display:flex;align-items:center;justify-content:center;
    background:linear-gradient(155deg,rgba(255,255,255,.1),rgba(255,255,255,.03));
    border:1px solid var(--border-hi); box-shadow:inset 0 1px 0 rgba(255,255,255,.18); transition:all .2s ease}
  .fmt-btn.active .ic{background:rgba(255,255,255,.22); border-color:rgba(255,255,255,.4);
    box-shadow:inset 0 1px 0 rgba(255,255,255,.35), 0 4px 10px rgba(0,0,0,.15)}
  ...
  .prog-pct{font-size:26px; font-weight:800; margin:2px 0 12px;
    background:var(--grad-full); -webkit-background-clip:text; background-clip:text; color:transparent}
  .prog-bar{width:100%; height:8px; border-radius:999px; background:rgba(255,255,255,.08);
    border:1px solid var(--border); overflow:hidden; margin-top:4px}
  .prog-fill{height:100%; border-radius:999px; background:var(--grad-full);
    box-shadow:0 0 12px rgba(111,141,255,.5); transition:width .5s ease}
  .prog-fill.indeterminate{width:35% !important; animation:prog-slide 1.3s ease-in-out infinite}
  @keyframes prog-slide{0%{margin-left:-35%}50%{margin-left:65%}100%{margin-left:-35%}}
    ...
    <div class="steps">
      <div class="step">
        ...
        <div class="source-row">
          ...
          <div class="source-btn" id="src-myvideos" onclick="selectSource('myvideos')"><span class="ic">🎞️</span>My Videos</div>
        </div>
...
<script>
  ...
  const STAGE_LABEL = {
    ...
    retrying: 'কিছু অংশ Skip করে আবার Convert করা হচ্ছে...',
  ...
  function renderProgress(stage, percent) {
    ...
    const known = (stage === 'converting' || stage === 'retrying') && pct > 0;
    document.getElementById('progressInner').innerHTML = `
      <div class="spinner"></div>
      ${known ? `<div class="prog-pct">${pct}%</div>` : ''}
      <div style="font-weight:700;margin-bottom:10px">${label}</div>
      <div class="prog-bar"><div class="prog-fill ${known ? '' : 'indeterminate'}" style="width:${known ? pct : 100}%"></div></div>
      <div style="color:var(--muted);font-size:12px;margin-top:10px">File-এর Size অনুযায়ী সময় লাগতে পারে</div>`;
  }
